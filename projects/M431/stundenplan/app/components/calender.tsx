...
export default function CalendarClient({ scheme, data }: CalendarClientProps) {
  ...
  const weeksWithDates = Object.entries(eventsByWeek).map(([week, days]) => {
    ...
    return {
      week: Number(week),
      year: earliestEvent.getFullYear(),
      earliestDate: earliestEvent,
      days,
    };
  });
  ...
  return (
    ...
    >
      <h1 className="mb-4 text-xl font-bold sm:text-2xl">Calendar Events</h1>
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              {nonEmptyDays.map((day) => {
                ...
                return (
                  ...
                  >
                    <h3 className="mb-2 text-center text-sm font-semibold sm:text-base">{day}</h3>
                    {days[day]
