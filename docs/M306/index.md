...
```mermaid
graph TD
    PL[Projektleitung] --> IT[IT / Entwicklung]
    PL --> VK[Verkauf / Fachbereich]
    PL --> MK[Marketing / Kommunikation]
    PL --> INF[IT-Infrastruktur]
```
...
```mermaid
graph TD
    AG[Auftraggeber: Hugo Koller, CEO] --> PA
    PA[Projektausschuss: Anton Sutter CIO, Henry Dupont, Bruno Abderhalden, Lara Gut] --> PL
    PL[Projektleitung: N.N. / IT-Abteilung] --> PT_IT
    PL --> PT_Fach
    PT_IT[Projektmitarbeiter IT: Plattformentwickler & Applikationsentwickler]
    PT_Fach[Projektmitarbeiter Fach: Vertreter Marketing, Hotellerie & Gesundheit]
```
